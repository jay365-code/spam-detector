...
{{
  ...
  "reason": "한 줄 근거(특이성/앵커/오탐위험)"
    ...
    print('Calling Gemini with full prompt simulation...')
    start = time.time()
    try:
        res = await asyncio.wait_for(client.ainvoke([SystemMessage(content=sys_prompt), HumanMessage(content=user_msg)]), timeout=120.0)
