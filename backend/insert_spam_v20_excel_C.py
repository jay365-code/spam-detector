...
EXCEL_PATH = r"d:\Projects\spam-detector\spams\MMSC스팸추출_20260107_C.xlsx"
SHEET_NAME = "육안분석(시뮬결과35_150)"

def insert_v20_excel_C():
    success_count = 0
    try:
        try:
            df = pd.read_excel(EXCEL_PATH, sheet_name=SHEET_NAME)
        except ValueError:
            print(f"[{SHEET_NAME}] 시트를 찾지 못했습니다. 첫 번째 시트를 로드합니다.")
        ...
        with sqlite3.connect(DB_PATH) as conn:
            ...
            print(f"=== 엑셀 C 파일 DB 업데이트 시작 (단일 타겟 문장: {len(unique_raw_texts)}건) ===")
            ...
            conn.commit()
        print(f"\n작업 완료! 엑셀 C 버전에서 정상 적용된 총 텍스트 수: {success_count}건")
