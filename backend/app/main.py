...
class RefineScanRequest(BaseModel):
    logs: dict = None
...
async def cluster_all_api(filename: str, request: ClusterAllRequest = None):
    ...
    프론트엔드에서 logs를 직접 전송하면 서버 파일 의존 없이 동작한다.
    """
    ...
    try:
        ...
        if request and request.logs:
            logger.info(f"'{filename}' 에 대한 전체 유사 메시지 클러스터 스캔을 시작합니다. (클라이언트 전송 데이터 사용)")
            data = {"logs": request.logs}
            _, clusters = ClusterService.find_all_similar_clusters(data=data)
        else:
            # 2순위: 서버 파일시스템 폴백 (로컬 개발 환경 호환)
            report_path = os.path.join(REPORTS_DIR, filename)
            if not os.path.exists(report_path):
                raise HTTPException(status_code=404, detail="Report not found (서버에 파일이 없고, 요청 body에 logs도 없음)")
            logger.info(f"'{filename}' 에 대한 전체 유사 메시지 클러스터 스캔을 시작합니다. (서버 파일 사용)")
            _, clusters = ClusterService.find_all_similar_clusters(report_path)
...
async def refine_scan_api(filename: str, request: RefineScanRequest = None):
    """
    ...
    try:
        if request and request.logs:
            logger.info(f"[SignatureRefiner] '{filename}' 에 대한 파편화 클러스터스캔을 시작합니다. (클라이언트 전송 데이터 사용)")
            input_data = {"logs": request.logs}
            _, clusters = ClusterService.find_target_clusters(data=input_data)
        else:
            report_path = os.path.join(REPORTS_DIR, filename)
            if not os.path.exists(report_path):
                raise HTTPException(status_code=404, detail="Report not found (서버에 파일이 없고, 요청 body에 logs도 없음)")
            logger.info(f"[SignatureRefiner] '{filename}' 에 대한 파편화 클러스터스캔을 시작합니다. (서버 파일 사용)")
            _, clusters = ClusterService.find_target_clusters(report_path)
...
async def refine_analyze_single_api(filename: str, request: RefineAnalyzeSingleRequest):
    ...
    이 엔드포인트는 서버 파일을 읽지 않으므로 파일 존재 여부 체크 불필요.
    """
...
async def refine_apply_api(filename: str, request: RefineApplyRequest):
    """
    UI에서 Confirm 완료된 시그니처 제안들을 적용.
    request.logs가 있으면 인메모리로 처리 후 수정된 엔트리 반환 (운영 서버 대응).
    없으면 서버 파일 기반으로 덮어쓰기 (로컬 개발 환경 호환).
    """
    import shutil
         
    try:
         use_body_logs = bool(request.logs)
         
         if use_body_logs:
             logger.info(f"[SignatureRefiner] 사용자 Confirm 수신. 클라이언트 전송 데이터 기반 인메모리 적용. (요청 클러스터 갯수: {len(request.clusters)})")
             logs = request.logs
         else:
             report_path = os.path.join(REPORTS_DIR, filename)
             if not os.path.exists(report_path):
              ...
              if cluster_obj.get("selected", False):
                  ...
                  for item in orig_items:
                       ...
                       if log_id in logs:
                            ...
                            modified_entries[log_id] = logs[log_id]
              else:
         ...
         if use_body_logs:
             logger.info(f"[SignatureRefiner] {applied_count}개 클러스터 인메모리 적용 완료. 수정된 {len(modified_entries)}개 로그 엔트리를 클라이언트에 반환합니다.")
             return {"success": True, "applied_clusters_count": applied_count, "modified_entries": modified_entries}
         else:
...
async def upload_file(client_id: str = Form(...), files: List[UploadFile] = File(...)):
    ...
    try:
        ...
        def process_message_with_hitl(messages: list, start_index: int = 0, total_count: int = 0, pre_parsed_urls: list = None, is_trap: bool = False) -> list:
            ...
            import asyncio
            from app.graphs.batch_flow import clear_batch_caches
            
            # [NEW] 이전 배치들의 시그니처 및 URL 런타임 캐시 완전 소각 (오염 전이 및 메모리 릭 방지)
            clear_batch_caches()
