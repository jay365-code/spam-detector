...
Step 4-3. 필터 회피용 벡터 노이즈 (Garbage Obfuscation) 판단 → is_garbage_obfuscation
   ...
   - **(주의/예외)** 단, '태싸ㅈr', '카찐오'처럼 몇몇 핵심 단어만 변형/난독화했을 뿐 전체적인 **문장 구조, 안부 인사, 홍보 문맥**이 뚜렷하게 도출되는 경우에는 false로 판독하라. 이 기준은 원본 문장의 50% 이상이 해독 불가능한 기호의 나열이어서 CNN 학습 시 도저히 문맥(Context)을 학습할 수 없을 정도로 훼손된 메시지일 때만 true가 된다.
...
Step 5. [CNN 모델 강건성(Robustness) 유지 안내]
   ...
   **판단 기준 (SPAM 확정 후 적용):** "이 메시지의 구조적 패턴이 CNN 학습에 사용될 경우, 향후 유사한 구조의 정상 메시지를 오탐(False Positive)할 위험이 있는가?"
   ...
   - Garbage: 문맥 해독이 불가한 무의미한 벡터 노이즈가 특정 맵을 왜곡 → 모델의 일반화 성능 저하 (단축키어/은어 변형은 해당 안 됨)
