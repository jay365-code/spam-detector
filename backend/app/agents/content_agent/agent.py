...
Step 4. [Type B 시그널 추출: CNN 모델 데이터 오염 방지 및 오탐(FP) 방어]
   ⚠️ 중요: 이 절차는 메시지가 SPAM으로 확정되었을 때만 수행한다. (HAM일 경우 모두 false 처리)
   메시지의 구조나 본문이 정교하게 '정상 메시지를 위장'했거나 반대로 심하게 '난독화/훼손'되어 있어, 이 텍스트를 정형 데이터로 CNN 모델에 학습시킬 경우 미래에 일반적인 정상 메시지들까지 스팸으로 오탐(False Positive)하게 만드는 부작용(Data Poisoning)을 초래하는지 평가하라.
   아래 조항(4-1 ~ 4-3) 중 하나라도 해당하면 강력하게 해당 시그널을 true로 반환한다.
   ...
   Step 4-3. 고의적 난독화 및 은닉 의도 (Intentional Obfuscation & Evasion) 판단 → is_garbage_obfuscation
      - 목적: 메시지의 텍스트가 스팸 필터를 우회할 목적(Evasion)으로 고의적으로 훼손, 암호화, 또는 난독화되어 있는가?
      - 범주: 특수기호 무단 삽입, 과도한 공백 및 분절, 초성 치환 등 그 '방식'에 관계없다.
      - 판단 기준: 텍스트에 쓰인 핵심 단어들이 정상 범용어("매주", "정산" 등)일지라도 그 조합의 형태가 문법/형태소를 비정상적으로 파괴하고 있다면 true이다. 이러한 텍스트를 CNN 토크나이저가 처리하게 되면 일반 단어들까지 스팸 특징으로 잘못 학습(Feature Contamination)하게 되므로 반드시 걸러내야 한다.
