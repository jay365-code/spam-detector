...
- **[🚨 도메인 및 단축 URL 단독 추출 규칙 (수정!)]** `naver.com` 이나 `bit.ly` 같은 도메인 본체만 달랑 단독 추출하는 것은 오탐을 유발하므로 절대 금지한다. **하지만 `bit.ly/3PLZCrx`나 `a.to/25zTe`처럼 고유 경로(Path)나 파라미터가 포함된 전체 식별 주소라면, 쓸데없는 랜덤 한글을 억지로 붙이지 말고 반드시 단축 URL 자체나 고유 Path 부분만을 단독으로 깔끔하게 추출해라! (중복 방지를 위함)**
