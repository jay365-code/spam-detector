...
1. **최우선 (Must Extract)**: ML을 우회하려는 시도 및 구조적 유도
    - **변형/은어**: '대.출', 'ㅋr톡', 'S.E.X', '은밀한 대화' 등 특수문자 삽입, 자모 분리.
    - **연락처/메신저 유도 문구 결합 (매우 중요)**: 전화번호(010)나 카톡 ID 유도가 있을 경우, **절대로 단순 번호만 포함된 짧은 후보를 고르지 마라 (타인 오탐 위험).** 반드시 **"유도 문장(예: 카톡 연락처 추가) + 전화번호 일부/전체"가 결합된 40바이트 후보(`use_40`)**를 선택하라. (예: `카톡 상단 사람플러스모양 클릭`, `카톡 연락처 추가 번호 : 010-8053`).
    - 전화번호나 ID 단독으로만 이루어진 후보는 오탐 위험이 높으므로 절대 선택하지 마라.
2. **차선 (High Priority)**: 확실하고 독특한 발신자 특정
    - **고유 식별 정보**: 특정 상담원 이름(예: '김미영 팀장', '신종훈 수석').
3. **보류/제외 (Consider Unextractable)**: 일반적인 스팸
    ...
    - **080 수신거부 번호 / 단순 일반 연락처**: 문맥 없이 번호만 존재하거나 공용 번호인 경우.

**[특수 규칙: URL/연락처가 포함된 경우 (Special Rule)]**
- **정상적인 형태의 URL**(`http`, `www`, `naver.com` 등 **깨끗한 형태**):
    - URL 차단 시스템이 별도로 처리하므로 IBSE 리소스 낭비를 막기 위해 **`unextractable`을 선택**한다.
- **난독화된/변형된 URL**(`vt⑨8g.COm`, `k-bank. xyz`, `me 2 . do` 등):
    - 정상 URL 필터를 우회하려는 악의적 변형이므로, 해당 변형 문자열 자체를 **시그니처로 추출(use_20 또는 use_40)해야 한다**.
