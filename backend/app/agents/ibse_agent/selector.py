...
1. **최우선 (Must Extract)**: 필터 우회 시도 및 고유 패턴
    - **변형/은어/난독화**: '대.출', 'ㅋr톡', 'S.E.X', '은밀한 대화', 'vt⑨8g' 등 특수문자 삽입, 자모 분리, 기이한 형태의 문자열. (가장 좋은 시그니처)
    - **특이한 맥락의 결합어**: 스팸 의도가 노골적인 문장과 고유 명사/단어가 결합된 40바이트 후보. (예: 특정 불법 사이트명, 특이한 은어 조합)
2. **차선 (High Priority)**: 발신자 보조 식별 정보 (단독 추출 절대 금지)
    - **이름/직함/전화번호 단독 추출 금지**: '김미영 팀장', '강남 광수', '010-1234'처럼 이름, 직함, 수식어, 부분 전화번호만 단독으로 추출하는 것은 절대 금지한다. 동명이인이나 무고한 정상 문자를 차단하는 심각한 오탐(False Positive)을 유발하기 때문이다.
    - 식별 정보(이름/번호 등)를 추출하려면 **반드시 특이 단어(은어/난독화/특수문자 조합)** 또는 **노골적인 스팸 의도 문장**과 결합된 긴 길이(예: 40바이트 후보 중 '김미영팀장상담010', '오피강남광수v!p')를 선택해야 한다. 결합된 뚜렷한 스팸성 후보가 없다면 과감히 `unextractable`을 선택한다.
3. **보류/제외 (Consider Unextractable)**: 일반적인 스팸
    - **단순 홍보/유도 문구**: "최고 수익 보장", "가입 시 혜택", "카톡 추가", "연락주세요", "무료 거부" 등. 누구나 정상적인 비즈니스나 안내에서 쓸 수 있는 말은 차단 리스트에 넣어도 효과가 낮거나 오탐을 낸다. -> **`unextractable` 권장**.
    - **080 수신거부 번호 단독 추출 금지**: 공용 번호이므로 단독으로 추출하지 않는다. 단, 발신자의 이름/업체명 등 **고유 문맥과 결합된 40바이트 형태(예: '강남광수무료수신거부080')로는 추출이 가능**하다.
