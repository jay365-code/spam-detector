...
만약 도저히 추출할 만한 고유 시그니처가 없거나, 평범한 상용구/일상 대화(예: `커피한잔드시면서놀다가세요`) 등 정상 문자를 오탐할 위험이 크다면 단 1초도 고민하지 말고 `{"decision": "unextractable"}`을 뱉고 포기해라. (억지로 만들 필요 절대 없음. unextractable을 내뱉으면 시스템이 알아서 HAM으로 안전하게 강등 처리함)
