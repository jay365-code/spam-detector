...
1. (최우선 타겟) 본문 내에 불법/악성 URL(커스텀 도메인 등)이 존재한다면 가급적 그 URL을 포함하여 시그니처를 추출해라. 
   - 🚫 **[예외 1: 정상 플랫폼 위장 방패막이]** 단축 URL(bit.ly 등)이나 범용 플랫폼/포털 도메인(naver.com, youtube.com 등), 뉴스 기사 도메인(newsbrite.net 등)은 절대 URL 단독으로 추출하지 마라! 전 국민 연쇄 차단이 발생한다. 이런 정상 도메인은 반드시 주변의 **악성 한글 문맥(어조, 찌라시 단어 등)**과 강하게 결합해서 추출해야 하며, 문맥과 결합할 수 없다면 아예 URL을 포기하라.
   - 🚫 **[예외 2: URL 중간 절단 엄금 (길이 초과)]** 만약 긴 URL을 문장형(40바이트)으로 추출하려고 할 때, 40바이트 제한 때문에 URL 도메인이나 Path 중간이 툭 잘려나간다면(예: `youtube.com/watc`처럼 끝부분이나 앞부분이 잘림), 오탐을 대폭 유발하므로 절대 추출해서는 안 된다! URL을 뽑는다면 끝까지 '풀(Full)'로 완전한 형태여야 하며, 제한 범위를 넘겨서 URL이 훼손된다면 **차라리 URL 추출을 완전히 포기하고** 메시지 본문에서 다른 Unique한 악성 한글 문구(예: `VIP 리딩방 폭등주 적중!!`)를 찾아 시그니처로 삼아라!
