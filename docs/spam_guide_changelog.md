...
**문제**
- "삼성전자 100주 무료 제공" 같은 명백한 피싱이 HAM으로 판정
- LLM이 "비현실적"이라고 인식했지만, "불확실하면 HAM" 원칙에 막힘
- 피싱/스미싱 의도 유형이 Guide에 명시되어 있지 않음

**spam_guide.md 변경**

1. **2.2 harm_anchor 판정 원칙** - 의도 유형 5가지 명시
   - 도박 의도
   - 성인/유흥 의도
   - 사기 의도
   - **피싱 의도** (신규): 비현실적 보상, 사칭, 긴급성 유발
   - 불법 금융 의도

2. **1.3 민원 방지 원칙** 수정
   - 기존: "불확실하면 HAM"
   - 변경: "**의도가** 불확실하면 HAM"
   - 추가: "비현실적 보상, 사칭, 긴급성 유발이 느껴지면 → 피싱 의도로 판단"
   - 예시 추가: "삼성 100주 무료 증정" → 피싱 의도 → SPAM

3. **5. CODE EVIDENCE RULES** - 피싱 카테고리 추가 (code "2")
   - 패턴 나열 대신 **의도 기반 판단**
   - 사칭 의도, 미끼 의도, 긴급성 의도, 정보탈취 의도

### 이유
- 패턴 나열 방식의 한계: 메시지 변형마다 추가 필요, 토큰 비용 증가
- LLM의 의도 파악 능력 활용: "비현실적"이라고 느꼈다면 그 의도가 무엇인지 판단하도록 유도
- 피싱/스미싱은 가장 빈번한 스팸 유형이므로 명시적 가이드 필요
