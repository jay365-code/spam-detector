...
| **SPAM** | **HAM** | `is_confirmed_safe` (transactional 아님) | **SPAM 유지 + URL Drop** (방패막이 — 정상 사이트 보호) |
| **SPAM** | **HAM** | `is_mismatched` (confirmed_safe 아님) | **SPAM 유지, URL 블랙리스트 보존** (사칭/가짜 사이트 의심) |
...
| `safe_injection` | 방패막이 Decoy URL — **`is_confirmed_safe=true`일 때만** 발동 (정상 사이트 보호) |
...
> **URL Drop vs 블랙리스트 보존 구분**: `is_confirmed_safe=true`(정상 사이트 확인)이면 Drop하여 정상 사이트 보호. `is_mismatched=true`이지만 `is_confirmed_safe=false`(미확인 사이트)이면 사칭/가짜 사이트 의심으로 블랙리스트에 보존.
> 사칭 사이트(`is_spam=True`)는 URL SPAM 분기(L524)에서 처리되므로 이 분기에 도달하지 않음.
