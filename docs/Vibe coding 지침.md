...
- **가상 환경 및 패키지 관리:** 파이썬 패키지 설치는 반드시 `./.venv` 가상 환경 상태에서 진행하며, 새로운 패키지 설치 후에는 관련 내용을 `requirements.txt`에 기록한다.
...
- **일관성 유지:** 프로젝트 내의 기존 코딩 스타일, 변수 명명 규칙, 아키텍처(예: 하이브리드 메모리 구조)를 엄격히 준수한다.
