...
with open("frontend/src/components/DatabaseManagerModal.tsx", "r", encoding="utf-8") as f:
    code = f.read()

# I will write a full script to output the rewritten version
...
export const DatabaseManagerModal: React.FC<DatabaseManagerModalProps> = ({ isOpen, onClose }) => {
  ...
  const sortedHistoryRecords = useMemo(() => {
    ...
    return sorted.filter(r => r.normalized_text.includes(historySearch));
...
with open("frontend/src/components/DatabaseManagerModal.tsx", "w", encoding="utf-8") as f:
    f.write(new_code)
print("done")
